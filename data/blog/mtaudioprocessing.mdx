...
Usually, you can directly use `AVAudioEngine`, but in order to interface with `AVComposition`'s `AVAudioMix`, there's precisely one inpoint: `MTAudioProcessingTap` on `AVAudioMix`.
...
Anyway, I wanted to try and use a wrapper from AudioKit to amke things less painful.
...
We embarrasingly reused a buffer. Lets take that out.
