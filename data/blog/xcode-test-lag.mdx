...
We wait about an hour for intruments to process the ten second (!!!) analysis stream and look at the results.
...
I'm starting to revise my assessment that vnodes -> files, though, because there's an absoluytely
...
althogh my money is on `DerivedData`, because that's where the build artifacts are stored.
