...
I first wrote subtree_for_node operating off of a string array slice and returning a `Result<D3Node, String>` object, but wasm_bindgen vomited on almost every part of that. The string array slice was instead passed in as a jsvalue and deservialized via serde, and the string error was serialized to js over serde as well.
