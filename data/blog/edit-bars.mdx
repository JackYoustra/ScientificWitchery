...
Whenever the movie is compiled (in our case, into an AVComposistion object, but you could imagine an ffmpeg call),
...
The target, on the other hand, was _not_ `self` but a completely separte instance of `PLUIEditVideoViewController`.
...
We can see that there's a `PLVideoView` in the view hiearchy
