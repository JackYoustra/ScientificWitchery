...
<div className="my-1 w-full overflow-hidden px-2 xl:my-1 xl:w-1/2 xl:px-2">
    ![Maple](/static/images/tca/elm.jpg)
</div>

The Elm architecture has three main concepts: the model (state), the view (a pure function that renders the UI), and the update function (a pure function that takes the current state and an action and returns a new state). These concepts are implemented in the three main components: the state, the actions, and the reducers. The reducers are pure functions that take the current state and an action and return a new state. The actions are enums that represent all the possible actions that can be performed on the state. The state is a struct that represents the current state of the application.
...
The 'better' way is just manual injection. It isn't that good - it's really, really verbose, but it's okay.\
...
We can use the compiler to verify that the two test functions yield the same results, and, indeed, [goldbolt shows](https://gcc.godbolt.org/z/sb9TWMq5Y) test is aliased to test2 in the injected solution (line 873 in the asm output).
...
Swift has no metaprogramming facilities at all. There's a very recent SE proposal for [proc_macros in swift](https://forums.swift.org/t/a-possible-vision-for-macros-in-swift/60900) but it's very early on and there's been a [shocking](https://forums.swift.org/t/a-possible-vision-for-macros-in-swift/60900/3) [amount](https://forums.swift.org/t/a-possible-vision-for-macros-in-swift/60900/7) [of](https://forums.swift.org/t/a-possible-vision-for-macros-in-swift/60900/22) [pushback](https://forums.swift.org/t/a-possible-vision-for-macros-in-swift/60900/74) for a system that has been _already tested and loved by the Rust community, which Swift has, for years, taken its cues from!_. I'm not sure why there's so much pushback, but I'm not going to get into that here.

For now, there may lie a solution in the hack that is Sourcery, which is a code generator that can generate code from templates. It's not a perfect solution, but it's a start.
